...
if ($_SESSION['authenticated'] === true) {
    if (isset($_POST['save_scores'])) {
        ...
        try {
            ...
            echo '<script>alert("No participants detected.");</script>';
        ...
        try {
            ...
            // Now, handle the mirroring of scores to similar participants
            // Check if the selected participant is part of a group (is_Grouped = 1)
            $checkGroupQuery = "SELECT organization_id, is_Grouped FROM participants WHERE participants_id = ?";
            $checkGroupStmt = $conn->prepare($checkGroupQuery);
            $checkGroupStmt->bind_param("i", $selectedParticipantId);
            $checkGroupStmt->execute();
            $checkGroupResult = $checkGroupStmt->get_result();
    
            if ($checkGroupRow = $checkGroupResult->fetch_assoc()) {
                $organizationId = $checkGroupRow['organization_id'];
                $isGrouped = $checkGroupRow['is_Grouped'];
                ...
                if ($isGrouped == 1) {
                    ...
                    // Insert the new mirror data
                    $mirrorQuery = "INSERT INTO criterion_scoring (ongoing_criterion_id, participants_id, criterion_final_score, judge_id)
                                    SELECT ?, participants_id, ?, ? FROM participants
                                    WHERE organization_id = ? AND is_Grouped = 1 AND participants_id != ?";
                    $mirrorStmt = $conn->prepare($mirrorQuery);
